...
class NeRFNetwork(NeRFRenderer):
    def __init__(self, 
        ...
        self.encoder, self.in_dim = get_encoder('hashgrid', input_dim=3, log2_hashmap_size=16, desired_resolution=1024 * self.bound)
        self.encoder_dir, self.in_dim_dir = get_encoder('sphere_harmonics', input_dim=3)
        self.sigma_net = MLP(self.in_dim, 1, hidden_dim, num_layers, bias=True)
        self.color_net = MLP(self.in_dim+self.in_dim_dir, 3, hidden_dim, num_layers, bias=True)
    ...
    def common_forward(self, x, d):
        ...
        h_d = self.encoder_dir(d)
        albedo = torch.sigmoid(self.color_net(torch.cat([h, h_d], -1)))
