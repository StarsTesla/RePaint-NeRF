...
class MSELoss(nn.Module):
    ...
    # TODO: modify this to feat
    def forward(self, inputs, targets):

        rgb_loss = self.loss(inputs, targets)
...
class Trainer(object):
    def __init__(self,
                 ...
                 pretrained=True,  # True for modeifed stage, False for pre-training stage
                 ):
    ...
    def prepare_text_embeddings(self):
        ...
        if not self.opt.dir_text:
            self.text_z = self.guidance.get_text_embeds(
                [self.opt.text], [self.opt.negative])
        else:
            self.text_z = []
    ...
    def train_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, staged=False, perturb=True, bg_color=None,
        ...
        gt_rgb = data['rgbs'].cuda().reshape(
            B, H, W, 3).permute(0, 3, 1, 2).contiguous()
        if self.pretrained:

            mask = data['mask'].cuda().reshape(B, H, W, 1).permute(
                0, 3, 1, 2).contiguous()
            mask_gt_rgb = gt_rgb * (mask < 0.5)
            mask_pred_rgb = pred_rgb * (mask < 0.5)

            # clip loss for black hole
            # background text
            bg_feature = self.clip_guidance.encode_img(mask_pred_rgb)
            bg_feature = bg_feature.clone() / bg_feature.norm(dim=-1, keepdim=True)
            text_feature = self.text_bg_z / \
                self.text_bg_z.norm(dim=-1, keepdim=True)
            similarity = text_feature @ bg_feature.T
            sim_loss = 1e-2 * -similarity[0][0]
            ...
            loss += 100 * self.rgb_loss(mask_pred_rgb, mask_gt_rgb)
            loss += sim_loss
        else:

            loss = self.rgb_loss(pred_rgb, gt_rgb)

        pred_ws = outputs['weights_sum'].reshape(B, 1, H, W)
        # pred_ws = pred_ws * (mask > 0.5)  # //TODO
    ...
    def test_step(self, data, bg_color=None, perturb=False):
        ...
        ambient_ratio = data['ambient_ratio'] if 'ambient_ratio' in data else 1.0
        light_d = data['light_d'] if 'light_d' in data else None
        # print("test_step befor rendering...............")
        outputs = self.model.render(rays_o, rays_d, staged=True, perturb=perturb, light_d=light_d,
                                    ambient_ratio=ambient_ratio, shading=shading, force_all_rays=True, bg_color=bg_color, **vars(self.opt))
    ...
    def test(self, loader, save_path=None, name=None, write_video=False):
        ...
        if write_video:
            ...
            imageio.mimwrite(os.path.join(
                save_path, f'{name}_rgb.mp4'), all_preds, fps=25, quality=8, macro_block_size=1)
            imageio.mimwrite(os.path.join(
    ...
    def test_gui(self, pose, intrinsics, W, H, bg_color=None, spp=1, downscale=1, light_d=None, ambient_ratio=1.0, shading='albedo'):
        ...
        # rays_o, rays_d = get_ndc_rays(rH, rW, )
